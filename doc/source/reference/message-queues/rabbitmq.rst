...
The management API is accessed via HAProxy, exposed only on the internal VIP.
As such, traffic to this API is encrypted when ``kolla_enable_tls_internal`` is
``true``. See :ref:`tls-configuration`.
